...
// ambil nilai tipe
const tipeMutasi = document.querySelector('#tpMutasi');
...
function filterData(){
    ...
    var filteredData = filterEachData(data, kategoriMutasi.value, tipeMutasi.value, rekTerpilih, start, end);
...
function filterEachData(data, category, tipe, rekening, startDate, endDate) {
    return data.filter(function(item) {
        if (item.kategori !== category && category!=="") {
            return false;
        }

        if (item.tipe !== tipe && tipe!=="") {
            return false;
        }
